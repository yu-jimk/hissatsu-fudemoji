...
{
    public class Judge_Kan : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            string judgingKan()
            {
                ...
                bool go1 = -130.0 < start.x && start.x < 1.0;
                bool go2 = 125.0 < start.y && start.y < 251.0;
                bool go3 = -229.0 < end.x && end.x < -49.0;
                bool go4 = -30.0 < end.y && end.y < 125.0;
                bool go5 = sumDistance < 230.0;
                sumDistance = 0.0;

                if (go1 & go2 & go3 & go4 & go5) //条件と比較
                {
