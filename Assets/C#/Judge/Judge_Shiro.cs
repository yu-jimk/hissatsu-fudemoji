...
{
    public class Judge_Shiro : MonoBehaviour
    {
        ...
        string judgingShiro(){
                ...
                bool hyaku1 = -200.0 < start.x && start.x < -27.0;
                bool hyaku2 = 105.0 < start.y && start.y < 230.0;
                bool hyaku3 = 0.0 < end.x && end.x < 190.0;
                bool hyaku4 = 125.0 < end.y && end.y < 250.0;
                bool hyaku5 = sumDistance < 500.0;

                bool ji11 = -140.0 < start.x && start.x < -40.0;
                bool ji12 = -11.0 < start.y && start.y < 47.0;
                bool ji13 = 40.0 < end.x && end.x < 85.0;
                bool ji14 = 1.0 < end.y && end.y < 47.0;
                bool ji15 = sumDistance < 200.0;

                bool ji21 = -120.0 < start.x && start.x < -40.0;
                bool ji22 = -110.0 < start.y && start.y < -10.0;
                bool ji23 = -40.0 < end.x && end.x < 85.0;
                bool ji24 = -110.0 < end.y && end.y < 0.0;
                bool ji25 = sumDistance < 200.0;

                sumDistance = 0.0;

                if (hyaku1 & hyaku2 & hyaku3 & hyaku4 & hyaku5) //条件と比較
                {
                    DataManager.Instance.isCorrect = 1;
                    DataManager.Instance.isdivide = 1;
                    DataManager.Instance.isScene = 1;
                    return "百";
                } else if (ji11 & ji12 & ji13 & ji14 & ji15)
                {
                    DataManager.Instance.isCorrect = 2;
                    DataManager.Instance.isdivide = 1;
                    DataManager.Instance.isScene = 1;
                    return "自";
                } else if (ji21 & ji22 & ji23 & ji24 & ji25)
                {
                    DataManager.Instance.isCorrect = 2;
                    DataManager.Instance.isdivide = 1;
                    DataManager.Instance.isScene = 1;
