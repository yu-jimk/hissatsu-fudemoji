...
{
    public class Judge_Ue : MonoBehaviour
    {
        ...
        string judgingUe(){
               ...
               nowflame = end;
                ...
                bool tomaru1 = -153.0 < start.x && start.x < 5.0;
                bool tomaru2 = -40.0 < start.y && start.y < 103.0;
                bool tomaru3 = -153.0 < end.x && end.x < 5.0;
                bool tomaru4 = -135.0 < end.y && end.y < 5.0;
                bool tomaru5 = sumDistance < 200.0;

                bool tuchi1 = -161.0 < start.x && start.x < -25.0;
                bool tuchi2 = -30.0 < start.y && start.y < 111.0;
                bool tuchi3 = -49.0 < end.x && end.x < 123.0;
                bool tuchi4 = -30.0 < end.y && end.y < 111.0;
                bool tuchi5 = sumDistance < 440.0;

                bool shi1 = -269.0 < start.x && start.x < -79.0;
                bool shi2 = -40.0 < start.y && start.y < 121.0;
                bool shi3 = 71.0 < end.x && end.x < 241.0;
                bool shi4 = -40.0 < end.y && end.y < 121.0;
                bool shi5 = sumDistance < 820.0;

                sumDistance = 0.0;

                if (tomaru1 & tomaru2 & tomaru3 & tomaru4 & tomaru5) //条件と比較
                {
                    DataManager.Instance.isCorrect = 1;
                    DataManager.Instance.isdivide = 1;
                    DataManager.Instance.isScene = 1;
                    return "止";
                } else if (tuchi1 & tuchi2 & tuchi3 & tuchi4 & tuchi5)
                {
