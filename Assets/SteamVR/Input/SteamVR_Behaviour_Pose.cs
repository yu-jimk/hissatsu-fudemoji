...
namespace Valve.VR
{
    ...
    public class SteamVR_Behaviour_Pose : MonoBehaviour
    {
        ...
        * ここから改造用のコード
        */
        /// <summary>VIVEトラッカーの移動距離が小さすぎるので増幅するための変数</summary>
        public float moveRange = 300;

        /// <summary>位置調整する際の基準座標</summary>
        ...
        protected virtual void UpdateTransform()
        {
            ...
            {
                /* --トラッカーの移動関連の記述はここ-- */
                transform.position = origin.transform.TransformPoint(poseAction[inputSource].localPosition)*moveRange;
                Vector3 Pos = transform.position;
                Pos.x = -Pos.x;
                Pos.z = Pos.z * 100 / moveRange;
                transform.position = Pos;

                SetPosDef(transform.position);
        ...
        /// <summary>
        /// Fキーを押したらデフォルトの位置に設定するやつ
        /// </summary>
        private void SetPosDef(Vector3 trackerPos){
            if(Input.GetKey(KeyCode.F)){
                diffPos.x = defPos.x - trackerPos.x;
                diffPos.y = defPos.y - trackerPos.y;
                diffPos.z = defPos.z - trackerPos.z;
            }
        }


        /// <summary>
        /// 位置リセット込みのトラッカーの結果的な位置情報を返すやつ
        /// </summary>
        private Vector3 SetPosition(Vector3 trackerPos){
            trackerPos.x += diffPos.x;
            trackerPos.y += diffPos.y;
            trackerPos.z += diffPos.z; //trackerPosのz軸座標をデフォルト（０）にする
            return trackerPos; //その状態で再度現在座標にtrackerPosの値を入れる
        }
